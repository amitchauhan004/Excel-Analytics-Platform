...
// Get uploaded files (for admin: all system files; for regular user: user's files)
router.get("/", authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user.id);
    const isAdmin = user && user.role === "admin";
    const query = isAdmin ? {} : { uploadedBy: req.user.id };

    const files = await FileMeta.find(query)
      .populate("uploadedBy", "name email role")
      .sort({ uploadedAt: -1 });

    // Populate missing fileSize dynamically from disk if needed
    const filesWithStats = files.map(file => {
      const fileObj = file.toObject();
      if (!fileObj.fileSize && fileObj.storedName) {
        const filePath = path.join(__dirname, "../uploads", fileObj.storedName);
        if (fs.existsSync(filePath)) {
          fileObj.fileSize = fs.statSync(filePath).size;
        }
      }
      return fileObj;
    });
